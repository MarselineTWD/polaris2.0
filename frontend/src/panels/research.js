...
const SOURCE_HELP = {
  ...
  space_track: {
    purpose: "Официальные сообщения CDM о сближениях и оценках вероятности столкновения.",
    effect: "уточнение риска после первичного отбора по каталогу CelesTrak",
    access: "Нужна отдельная учётная запись",
    cadence: "не подключён · источник необязательный",
  },
  discos: {
    purpose: "Масса, размеры, форма и эффективная площадь каталогизированных космических объектов.",
    effect: "уточнение сопротивления атмосферы и параметров сближения",
    access: "Нужна учётная запись ESA",
    cadence: "не подключён · источник необязательный",
  },
};
  ...
  return `
    <section class="research-section source-section">
      ...
      <div class="source-access-note"><strong>Почему доступ требуется не везде?</strong>
        Open-Meteo, NOAA, CelesTrak и SatNOGS дают публичное чтение без ключа. ITU-Rpy и SatKit уже находятся внутри Docker-образа.
        Space-Track и ESA DISCOS ограничивают доступ условиями своих каталогов, поэтому без учётных записей они не запрашиваются
        и не блокируют расчёт.</div>
    </section>`;
}
...
function sourceStatusMeaning(source) {
  return ({
    ...
    requires_auth: "Источник не подключён и в текущий расчёт не входит.",
...
function statusLegend() {
  return `<div class="source-status-legend" aria-label="Обозначения статусов источников">
    ...
    <span><i class="muted"></i><b>нужен доступ</b> — необязательный источник не подключён</span>
  </div>`;
}
